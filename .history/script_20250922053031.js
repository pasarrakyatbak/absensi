...
// =================== Load semua lapak (sinkron DB) ===================
async function loadLapak() {
    showAbsensiLoading(true, "Sedang memuat data lapak...");

    try {
        // ✅ pakai endpoint lapakHariIni supaya status hadir permanen
        ...
        // Ambil langsung data dari server, sudah ada statusAbsensi
        lapakData = json.data;
        ...
        console.log("Lapak data loaded:", lapakData);
...
function openAbsensiModal(noLapak, namaLapak) {
    ...
    if (modal && info && container) {
        info.textContent = `Absen untuk Lapak ${noLapak} - ${namaLapak}`;
        container.classList.add("blur");    // tambahkan blur
        document.body.classList.add("modal-open"); // hentikan scroll
        modal.style.display = "flex";        // tampilkan modal
        setTimeout(() => modal.classList.add("show"), 10); // fade + scale
    }
}
...
function closeAbsensiModal() {
    ...
    container.classList.remove("blur");    // hilangkan blur
    document.body.classList.remove("modal-open"); // aktifkan scroll

    // delay display none agar animasi selesai
    setTimeout(() => {
...
function renderLapak(data) {
    ...
    `;
        ...
        if (!statusHadir && cardEl) {
            // Hanya buka modal, jangan langsung absen
...
// =================== Absen ke server (UI lokal update) ===================
...
if (modalAbsenBtn) {
    modalAbsenBtn.addEventListener("click", () => {
        if (currentLapakId) {
            absenLapak(currentLapakId); // panggil fungsi absen
...
function updateBlurOnScroll() {
    ...
    if (modal.classList.contains("show")) {
        ...
        // blur dinamis: semakin scroll jauh, blur tetap smooth
