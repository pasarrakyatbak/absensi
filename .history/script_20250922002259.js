...
async function loadLapak() {
    try {
        const res = await fetch(API_URL + "?action=listLapak"); // endpoint listLapak
...
function renderLapak(data) {
    ...
    data.forEach(lapak => {
        // beri default jika statusAbsensi undefined
...
async function absenLapak(noLapak) {
    ...
    try {
        ...
        if (!json.success) {
            ...
            // Refresh otomatis, beri default jika belum ada properti statusAbsensi
