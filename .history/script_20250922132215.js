...
function renderLapak(data) {
    ...
    data.forEach(lapak => {
        ...
        card.innerHTML = `
    <div class="card lapak-card shadow-sm ${hadir ? 'hadir disabled' : 'belum'}" 
...
async function hapusAbsensiLapak(noLapak) {
    ...
    try {
        ...
        if (!json.success) {
            alert(json.message || "❌ Gagal menghapus absensi.");
        } else {
            const lapakIndex = lapakData.findIndex(l => String(l.noLapak) === String(noLapak));
...
const modalHapusBtn = document.createElement("button");
modalHapusBtn.id = "modalHapusBtn";
modalHapusBtn.textContent = "Hapus Absensi";
modalHapusBtn.className = "btn btn-danger mt-2";

modalHapusBtn.addEventListener("click", async () => {
    if (!currentLapakId) return;
...
// Masukkan tombol ke modal (di bawah tombol absen)
const modalContent = document.querySelector("#absensiModal .modal-content");
if (modalContent && !document.getElementById("modalHapusBtn")) {
    modalContent.insertBefore(modalHapusBtn, modalContent.querySelector(".btn-secondary"));
}
