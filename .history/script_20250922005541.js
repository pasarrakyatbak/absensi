...
function updateSummary() {
    ...
    const totalBelum = lapakData.length - totalHadir;
    const persenHadir = lapakData.length > 0
        ? Math.round((totalHadir / lapakData.length) * 100)
        : 0;
    ...
    if (summaryEl) {
        summaryEl.innerHTML = `
            <div class="summary-card total">
                Total<br>
                <span style="font-size:1.4rem;">${lapakData.length}</span>
            </div>
            <div class="summary-card hadir">
                ✅ Hadir<br>
                <span style="font-size:1.4rem;">${totalHadir}</span>
                <div style="font-size:0.9rem; opacity:0.8;">(${persenHadir}%)</div>
            </div>
            <div class="summary-card belum">
                ❌ Belum<br>
                <span style="font-size:1.4rem;">${totalBelum}</span>
            </div>
        `;
    }
}
